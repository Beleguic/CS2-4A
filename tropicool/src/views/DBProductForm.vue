    ...
    <div class="py-8 px-6">
      ...
      <form v-if="mode !== 'delete'" @submit.prevent="submitForm" class="grid gap-6" enctype="multipart/form-data">
        ...
        <div class="grid gap-1">
          ...
          <input type="text" id="reference" v-model="product.reference"
            class="p-2 block w-full border border-gray-300 rounded-md shadow-sm" required />
        </div>
        <div class="grid gap-4 grid-cols-2">
          <div>
            <label for="price" class="block text-sm font-medium text-gray-700">Prix</label>
            <input type="number" step="0.01" id="price" v-model.number="product.price"
              class="p-2 block w-full border border-gray-300 rounded-md shadow-sm" required min="0" />
          </div>
          <div>
            <label for="tva" class="block text-sm font-medium text-gray-700">TVA (%)</label>
            <input type="number" step="0.01" id="tva" v-model.number="product.tva"
              class="p-2 block w-full border border-gray-300 rounded-md shadow-sm" required min="0" />
          </div>
        </div>
        <div class="grid gap-1">
          ...
          <textarea id="description" v-model="product.description"
            class="p-2 block w-full border border-gray-300 rounded-md shadow-sm" required></textarea>
        </div>
        <div class="grid gap-1">
        ...
        <div class="grid gap-1">
          ...
          <input type="checkbox" id="is_active" v-model="product.is_active"
            class="p-2 block border border-gray-300 rounded-md shadow-sm" />
        </div>
        <div class="grid gap-1">
          <label for="is_adult" class="block text-sm font-medium text-gray-700">Produit réservé aux adultes</label>
          <input type="checkbox" id="is_adult" v-model="product.is_adult"
            class="p-2 block border border-gray-300 rounded-md shadow-sm" />
        </div>
        <button type="submit" class="px-4 py-2 bg-main text-white rounded-md hover:bg-secondary">{{ mode === 'new' ?
...
const submitForm = async () => {
  try {
    ...
    if (response.ok) {
      const eventName = `product-${mode.value === 'new' ? 'added' : 'updated'}`;
