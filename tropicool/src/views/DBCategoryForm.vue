<template>
    <section class="h-full">
      <div class="py-8 px-6">
        ...
        <h1 v-if="mode === 'edit'" class="text-4xl font-bold mb-8 text-black">Édition de la catégorie : <span class="capitalize">{{ formData?.name }}</span></h1>
  ...
  const submitForm = async (formDataFromChild: Category) => {
    formData.value = formDataFromChild;
    ...
    try {
      ...
      const { id, createdAt, updatedAt, created_at, updated_at, ...payload } = formData.value;
