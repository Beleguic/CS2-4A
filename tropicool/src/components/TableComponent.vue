<template>
  <div :style="{ maxWidth: dynamicMaxWidth, maxHeight: dynamicMaxHeight }" class="overflow-auto">
    <table class="w-full text-left table-auto relative">
      ...
      <tbody ref="tbodyRef">
        <tr v-for="(data, rowIndex) in datas" :key="data._id" :ref="el => trRefs[rowIndex] = el"
          :class="{
          ...
          <td v-for="(column, index) in columns" :key="index"
            ...
            <template v-else>
              <div class="flex items-center gap-4">
                <router-link v-if="newLink" :to="{ name: newLink, params: { id: data._id } }" class="p-4 bg-green-500 hover:bg-green-800 rounded-md flex flex-col justify-center items-center text-white w-16 h-16">
                  <component :is="iconAdd" />
                </router-link>
                <router-link v-if="viewLink" :to="{ name: viewLink, params: { id: data._id } }" class="p-4 bg-yellow-500 hover:bg-yellow-800 rounded-md flex flex-col justify-center items-center text-white w-16 h-16">
                  <component :is="iconEye" />
                </router-link>
                <router-link v-if="editLink" :to="{ name: editLink, params: { id: data._id } }" class="p-4 bg-blue-500 hover:bg-blue-800 rounded-md flex flex-col justify-center items-center text-white w-16 h-16">
                  <component :is="iconPen" />
                </router-link>
                <router-link v-if="deleteLink" :to="{ name: deleteLink, params: { id: data._id } }" class="p-4 bg-red-500 hover:bg-red-800 rounded-md flex flex-col justify-center items-center text-white w-16 h-16">
                  <component :is="iconTrash" />
                </router-link>
                <router-link v-if="graphLink" :to="{ name: graphLink, params: { id: data._id } }" class="p-4 bg-purple-500 hover:bg-purple-800 rounded-md flex flex-col justify-center items-center text-white w-16 h-16">
...
const { datas, columns, editLink, deleteLink, newLink, viewLink, graphLink } = props;
